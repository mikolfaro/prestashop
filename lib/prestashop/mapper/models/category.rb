...
module Prestashop
  module Mapper
    class Category < Model
      ...
      attr_accessor :id, :id_lang, :id_parent, :level_depth, :active,
                    :id_shop_default, :is_root_category, :position
      ...
      # Find category by name and id parent,
      # create new one from hash, when doesn't exist
      def find_or_create
      ...
      class << self
        # Search for category based on args on cached categories,
        # see #cache and #Client::Settings.categories_cache
        # Returns founded category or nil
        #
        def find_in_cache(id_parent, name, id_lang)
          Client.categories_cache.find do |c|
            c[:id_parent] == id_parent && c[:name].find_lang(name, id_lang)
          end
        end
        ...
        # Create new category based on given param,
        # delimited by delimiter in settings
        #
        # ==== Example:
        ...
        def create_from_name(category_name, id_lang)
          ...
          id_parent = 2
          names.each do |name|
            id_parent = new(name: name,
                            id_parent: id_parent,
                            id_lang: id_lang).find_or_create
